...
class AdversarialReweightedModel(BaseModel):
    ...
    def __init__(self,
        ...
        self.adversary_optimizer = torch.optim.Adam(self.parameters(), lr=1e-4)
...
class ImprovedModel(BaseModel):
    ...
    def calculate_loss(self, x, y, return_losses=False):
        ...
        discriminating_loss = torch.abs(pearsons_corr(learner_loss.detach(), self.adversary_network(x).detach().reshape(len(x))))\
                * self.discriminator_mult
    ...
    def calc_adversary_loss(self, x, y):
        learner_loss = self.cross_entropy(self.learner_forward(x), y)
        discriminating_loss = pearsons_corr(learner_loss.detach(), self.adversary_network(x).reshape(len(x)))
        #
        # discriminating_loss = (
    ...
    def fit(self, x, y, val_x=None, val_y=None, epochs=500, batch_size=64):
        ...
        for e in (range(epochs)):
            ...
            count += 1
            if count >= self.switch_count:
                count = 0
                training_learner = not training_learner
